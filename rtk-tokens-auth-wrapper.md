...
I like storing tokens in the cookies because they have expiration dates, security flags such as `httpOnly`, `SameSite` and `Secure` which make them less accessible to malicious actors whereas local storage is accessible to any script running on the page.
